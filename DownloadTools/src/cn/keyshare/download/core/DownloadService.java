...
public class DownloadService extends Service {
    ...
    private boolean mHaveDownloadWaiting = false;
	...
	public void run() {
		...
		synchronized (DownloadService.this) {
		    ...
		    if (!mPendingUpdate&&!mHaveDownloadWaiting) {
			mUpdateThread = null;
			if (!keepService) {
			    stopSelf();
			}
	...
	boolean justPaused = ((oldStatus == Downloads.STATUS_RUNNING
			||oldStatus == Downloads.STATUS_PENDING)
			&& Downloads.isStatusPaused(info.mStatus));
	
	if (justCompleted||justPaused){
		DownloadInfo.removeIdIfExist(info.mId);
	}
