...
import cn.keyshare.downloadtools.R;
...
class DownloadNotification {
    ...
    HashMap <Long, Notification> mShowingNotification = new HashMap<Long, Notification>();
        ...
        for (DownloadInfo download : downloads) {
            ...
            if (title == null || title.length() == 0) {
                title = mContext.getResources().getString(
                        R.string.keyshare_download_download_unknown_title);
            }
...
//                item.mDescription = download.mDescription;
            ...
            if (download.mStatus == Downloads.STATUS_QUEUED_FOR_WIFI
                ...
                item.mPausedText = mContext.getResources().getString(
                        R.string.keyshare_download_notification_need_wifi_for_size);
            }
        }
        HashMap <Long, Notification> upDatedNotification = new HashMap<Long, Notification>(); 
        // Add the notifications
        for (NotificationItem item : mNotifications.values()) {
            // Build the notification object
        	
            Notification n = mShowingNotification.get(Long.valueOf(item.mId));
            if(n == null){
            	n = new Notification();
            }
...
//            n.flags |= Notification.FLAG_ONGOING_EVENT;
            ...
            RemoteViews expandedView = n.contentView;
            if(expandedView == null){
             expandedView = new RemoteViews(mContext.getPackageName(),
                    R.layout.dt_status_bar_ongoing_event_progress_bar);
            }
            StringBuilder title = new StringBuilder(item.mTitles[0]);
            if (item.mTitleCount > 1) {
                title.append(mContext.getString(R.string.keyshare_download_notification_filename_separator));
                title.append(item.mTitles[1]);
                n.number = item.mTitleCount;
                if (item.mTitleCount > 2) {
                    title.append(mContext.getString(R.string.keyshare_download_notification_filename_extras,
            ...
            upDatedNotification.put(Long.valueOf(item.mId), n);
            
            mSystemFacade.postNotification(item.mId, n);

        }
        mShowingNotification = upDatedNotification;
    }
        ...
        for (DownloadInfo download : downloads) {
            ...
            if (Downloads.isStatusError(download.mStatus)) {
                caption = mContext.getResources()
                        .getString(R.string.keyshare_download_notification_download_failed);
                intent = new Intent(Constants.ACTION_LIST);
            } else {
                caption = mContext.getResources()
                        .getString(R.string.keyshare_download_notification_download_complete);
