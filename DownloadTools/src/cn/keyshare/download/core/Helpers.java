...
public class Helpers {
	...
	public static void checkDownloadDir(String basePath)
	...
	private static String getPathForFileUri(String url, String hint,
		...
		if (!isExternalMediaMounted(path)) {
			throw new GenerateSaveFileError(
					Downloads.STATUS_WAITING_FOR_STORAGE_DEVICE,
		...
		if (path.endsWith("/")) {
			...
			if (new File(basePath).exists() && !(new File(basePath).isDirectory())) {
				...
				throw new GenerateSaveFileError(
						...
						"requested destination file already exists, the file have the same name as directory");
			}
			path = generateFilePath(basePath, url,hint, contentDisposition,
					contentLocation, mimeType, destination, contentLength);
		} else {
		...
		if (getAvailableBytes(getFilesystemRoot(path)) < contentLength) {
			throw new GenerateSaveFileError(
					...
					"insufficient space on external storage or download path error");
		}
	...
	public static File getFilesystemRoot(String path) {
		...
		String externalSecond = SdCardUtils.getSecondExterPath() + "/";
		
		if(path.startsWith(externalSecond)){
			return new File(externalSecond);
		}
	...
	private static String generateFilePath(String basePath, String url,String hint,
			String contentDisposition, String contentLocation, String mimeType,
			int destination, long contentLength) throws GenerateSaveFileError {

		checkDownloadDir(basePath);
		...
		String uniqueFileName = chooseUniqueFilename(destination, filename,
				extension, recoveryDir);

		return uniqueFileName + Constants.TEMP_FILE_EXTENTION;
	}
	...
	private static String chooseFullPath(Context context, String url,
		...
		return generateFilePath(base.getPath(), url,hint, contentDisposition,
				contentLocation, mimeType, destination, contentLength);
	}
	...
	private static File getExternalDestination(long contentLength)
			throws GenerateSaveFileError {
		if (!isExternalMediaMounted(null)) {
			throw new GenerateSaveFileError(
					Downloads.STATUS_WAITING_FOR_STORAGE_DEVICE,
					"external media not mounted");
		}
	...
	//判断sd卡是否可用了, fileName == null 表示 判断两个sd卡, 否则判断fileName对应的那个sd卡, 当内置sd卡卸载时,外置sd卡一定卸载
	public static boolean isExternalMediaMounted(String fileName) {
		...
		if (fileName == null
			...
			return SdCardUtils.isSecondSDcardMounted();
		}
	...
	private static String chooseExtensionFromMimeType(String mimeType,
		...
		if (extension == null) {
			if (mimeType != null
					&& mimeType.toLowerCase(Locale.US).startsWith("text/")) {
	...
	public static boolean isFilenameValid(String filename) {
		...
		String sd2 = SdCardUtils.getSecondExterPath();
		// slashes
		return filename.startsWith(Environment.getDownloadCacheDirectory()
				.toString())
				|| filename.startsWith(Environment
						.getExternalStorageDirectory().toString())||(sd2==null||filename.startsWith(sd2));
	}
	...
	/* package */static void deleteFile(ContentResolver resolver, long id,
		...
		try {
			FileOperateUtil.deleteFileOrDir(path);
		} catch (Exception e) {
