...
public final class Downloads implements BaseColumns {
     ...
     */
    public static final String AUTHORITY = AppData.initAuthority();
   ...
   public static final String COLUMN_ICON_URL = "icon_url";
     ...
     * @hide
     */
    public static final int DESTINATION_EXTERNAL = 0; //这个代码路径,暂时没有用到,请不要考虑这条路径
    ...
    public static boolean isStatusPaused(int status){
    	return (status==Downloads.STATUS_PAUSED_BY_APP
    			||status == Downloads.STATUS_WAITING_TO_RETRY
    			||status == Downloads.STATUS_WAITING_FOR_NETWORK
    			||status == Downloads.STATUS_QUEUED_FOR_WIFI
    			||status == Downloads.STATUS_WAITING_FOR_STORAGE_DEVICE
    			);
    }
    ...
    //等待启动中,如果有多个任务时,等待其他任务先下载完
    public static final int STATUS_WAITING = 189;
    ...
    //内存卡被拔出,等待内存卡插入
    public static final int STATUS_WAITING_FOR_STORAGE_DEVICE = 197;
