...
public class DownloadManager {
   ...
   public static final String COLUMN_ICON_URL = "icon_url";
    ...
    public final static int STATUS_WAITING = 1 << 5;//等待启动下载
    ...
    private static final String[] COLUMNS = new String[] { COLUMN_ID,
	    COLUMN_TITLE, COLUMN_DESCRIPTION, COLUMN_ICON_URL, COLUMN_URI, COLUMN_MEDIA_TYPE,
    ...
    private static final String[] UNDERLYING_COLUMNS = new String[] {
	    ...
	    Downloads.COLUMN_DESCRIPTION, Downloads.COLUMN_ICON_URL, Downloads.COLUMN_URI,
	...
	private int mShowNotificationType = Downloads.VISIBILITY_VISIBLE_NOTIFY_COMPLETED;
	...
	//如果文件名为空,则自动生成文件名,
	public Request setDestinationPath(String filePath,String fileName){
	    if (filePath == null) {
	    ...
	    mDestinationUri =  Uri.parse("file://" + filePath);
	    if(fileName != null){
	    	mDestinationUri = Uri.withAppendedPath(Uri.fromFile(new File(filePath)), fileName);
	    }
	...
	//设置用于显示图片的URL
	public Request setIconUrl(CharSequence iconUrl){
		mIcon_url = iconUrl;
		return this;
	}
	...
	/**
	 * 设置通知栏显示类型,默认为Downloads.VISIBILITY_VISIBLE_NOTIFY_COMPLETED
	 */
	public Request setShowRunningNotification(int showType) {
		mShowNotificationType = showType;
	    return this;
	}
	...
	Cursor runQuery(ContentResolver resolver, String[] projection,
		...
		if((mStatusFlags & STATUS_WAITING) != 0) {
			parts.add(statusClause("=", Downloads.STATUS_WAITING));
		}
...
//		 if (status != STATUS_RUNNING && status != STATUS_PENDING && status != STATUS_WAITING && status != STATUS_PAUSED) {
//		    throw new IllegalArgumentException(
//			    "Can only pause a running download: "
//				    + cursor.getLong(cursor
//					    .getColumnIndex(COLUMN_ID)) + " current status is " + status);
//		}
	    }
	} finally {
	    cursor.close();
	...
	if(status == STATUS_RUNNING || status == STATUS_PENDING || status == STATUS_WAITING){
		ContentValues values = new ContentValues();
	...
	try {
		...
		status = cursor
			.getInt(cursor.getColumnIndex(COLUMN_STATUS));
//		if (status != STATUS_PAUSED && status != STATUS_WAITING && status != STATUS_RUNNING && status != STATUS_PENDING) {
//		    throw new IllegalArgumentException(
...
//		}
	...
	if(status == STATUS_PAUSED){
		ContentValues values = new ContentValues();
		values.put(Downloads.COLUMN_STATUS, Downloads.STATUS_WAITING);		
...
//					    .getColumnIndex(COLUMN_ID)));
			//如果界面上面点的太快,状态切换不过来时,就会发生问题,所以此时不要抛出异常为好
//			return ;
		}else{
			setIDs.add(id);
			String fileName = cursor.getString(cursor.getColumnIndex(COLUMN_LOCAL_URI));
			if(fileName!=null){
	...
	Long[] setIdsArray = setIDs.toArray(new Long[0]);
	
	long[] setIdlong = new long[setIdsArray.length];
	
	for(int i=0;i<setIdsArray.length;++i){
		setIdlong[i] = setIdsArray[i];
	}
...
//	values.putNull(Downloads._DATA);
	...
	mResolver.update(mBaseUri, values, getWhereClauseForIds(setIdlong),
		getWhereArgsForIds(setIdlong));
    }
	...
	private String translateString(String column) {
	    ...
	    if (column.equals(COLUMN_ICON_URL)){
	    return getUnderlyingString(Downloads.COLUMN_ICON_URL);
	    }
	...
	private int translateStatus(int status) {
	    ...
	    case Downloads.STATUS_WAITING_FOR_NETWORK:
	    case Downloads.STATUS_QUEUED_FOR_WIFI:
	    case Downloads.STATUS_WAITING_FOR_STORAGE_DEVICE:
